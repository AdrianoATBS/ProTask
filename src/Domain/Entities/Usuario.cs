...
public class Usuario
{
    ...
    public Usuario(Guid hash, string nome, string email, string cargo, bool statusDeAceitacao, DateTime dataDeCriacao, DateTime ultimoLogin)
    {
        ...
        // Validação do último login
        var resultadoUltimoLogin = Validacao.ValidarUltimoLogin(ultimoLogin);
        if(!resultadoUltimoLogin.IsValid)
            throw new ValidacaoException(resultadoUltimoLogin.ErrorMessage, 106);
